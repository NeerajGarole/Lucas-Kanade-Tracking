...
rect_0 = np.array([59, 116, 145, 151])
rect = np.copy(rect_0)
...
p_temp = [0.0, 0.0]
p = np.array(p_temp)
Pn_1 = np.array(p_temp)
for i in tqdm(range(1, seq.shape[2])):
    ########## TODO Implement LK with drift correction ##########
    ########## TODO Implement LK with drift correction ##########

    pn = LucasKanade(seq[:, :, i - 1], seq[:, :, i], rect, threshold, num_iters, Pn_1)
    p = p + pn
    Pn_ = LucasKanade(It0, seq[:, :, i], template_rect, threshold, num_iters, p)
    Pn_arr = np.array((Pn_[0], Pn_[1], Pn_[0], Pn_[1]))
    val = abs(np.sum(Pn_ - p - pn))
    if val < threshold_drift:
        rect = template_rect + Pn_arr
        Pn_1 = Pn_ - p
    else:
        for j in range(4):
            rect[j] = rect[j] + pn[j % 2]
        Pn_1 = pn
    lk_res.append(rect)
